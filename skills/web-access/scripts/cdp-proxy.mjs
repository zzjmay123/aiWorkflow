    ...
    if (ok) {
      ...
      try {
        ...
        if (versionData.webSocketDebuggerUrl) {
          // 提取 wsPath (如 /devtools/browser/xxx-xxx)
          const wsPath = new URL(versionData.webSocketDebuggerUrl).pathname;
          console.log(`[CDP Proxy] 扫描发现 Chrome 调试端口：${port} (带 wsPath)`);
          return { port, wsPath };
        }
      } catch { /* 忽略错误，回退到无 wsPath */ }
    ...
    // GET /info?target=xxx - 获取页面信息
      ...
      res.end(JSON.stringify({
        ...
        endpoints: {
          ...
          '/click?target=': 'POST body=CSS 选择器 - 点击元素',
